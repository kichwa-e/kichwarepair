...
def test_form_validation(page: Page):
    ...
    name_input = page.locator('form[name="quote"] input[name="name"]')
    phone_input = page.locator('form[name="quote"] input[name="phone"]')
    issue_textarea = page.locator('form[name="quote"] textarea[name="issue"]')

    expect(name_input).to_have_attribute('maxlength', '100')
    expect(phone_input).to_have_attribute('maxlength', '30')

    # Evaluate pattern attribute due to unicode issues in playwright python
