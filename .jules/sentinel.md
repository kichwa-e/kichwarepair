...
**Vulnerability:** Forms using Netlify integration lacked client-side input validation like length limits and character constraints.
**Learning:** Because backend validation is handled entirely by Netlify, oversized payloads and malformed data can easily be submitted without immediate frontend rejection.
**Prevention:** Always implement robust client-side validation (e.g., `maxlength`, `pattern`) as the first line of defense when using serverless form solutions.
