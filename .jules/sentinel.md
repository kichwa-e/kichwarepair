...
## 2024-05-01 - CSP Script-Src Hardening
**Vulnerability:** Inline scripts required 'unsafe-inline' in the script-src CSP directive, creating an XSS risk.
**Learning:** Even when using CDN-based tools like Tailwind Play, configuration and initialization scripts can be externalized to separate files. This completely removes the need for 'unsafe-inline' in the script-src policy.
**Prevention:** Always externalize configuration and initialization scripts when using CDNs to maintain a strict script-src policy without 'unsafe-inline'.
